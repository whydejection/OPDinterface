...
def read_data_range_streaming(
    ...
    *,
    ...
    def _normalize_preview(arr):
        ...
        if p98 <= 0.0:
            p98 = 1.0
        return np.clip(a / p98, -1.0, 1.0).astype(np.float32, copy=False)
    ...
    with segyio.open(_segyio_path(path), "r", ignore_geometry=True, strict=False) as f:
        ...
        for offset in range(0, selected_traces, chunk_size):
            ...
            chunk_matrix = np.asarray(f.trace.raw[from_trace:to_trace:step])
            if chunk_matrix.dtype != np.float32:
                chunk_matrix = chunk_matrix.astype(np.float32, copy=False)
            ...
            while preview_cursor < len(preview_idx) and int(preview_idx[preview_cursor]) < part_end:
                ...
                if 0 <= local_row < chunk_matrix.shape[0]:
                    preview_rows.append(chunk_matrix[local_row : local_row + 1, :].copy())
                preview_cursor += 1
        ...
        preview_matrix_norm = _normalize_preview(preview_matrix)
        ...
        return {
            ...
            "preview_matrix_norm": preview_matrix_norm,
    ...
    *,
    ...
    with segyio.open(_segyio_path(path), "r", ignore_geometry=True, strict=False) as f:
        ...
        for offset in range(0, selected_traces, chunk_size):
            ...
            from_trace = start + offset * step
            to_trace = start + part_end * step
            current_chunk = np.asarray(f.trace.raw[from_trace:to_trace:step])
            if current_chunk.dtype != np.float32:
                current_chunk = current_chunk.astype(np.float32, copy=False)

            source_preview_rows = {}
            while preview_cursor < len(preview_idx) and int(preview_idx[preview_cursor]) < part_end:
                local_row = int(preview_idx[preview_cursor]) - offset
                if 0 <= local_row < current_chunk.shape[0]:
                    source_preview_rows[preview_cursor] = current_chunk[local_row : local_row + 1, :].copy()
                preview_cursor += 1
            ...
            for k, before_row in source_preview_rows.items():
                local_row = int(preview_idx[k]) - offset
                if 0 <= local_row < current_chunk.shape[0]:
                    preview_before_rows.append(before_row)
                    preview_after_rows.append(current_chunk[local_row : local_row + 1, :].copy())
    ...
    before_plot_norm = _normalize_preview(before_plot)
    after_plot_norm = _normalize_preview(after_plot)
    return {
        "max_abs": max_abs,
        "before_preview": before_plot,
        "after_preview": after_plot,
        "before_preview_norm": before_plot_norm,
        "after_preview_norm": after_plot_norm,
    }
