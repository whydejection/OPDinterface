...
class App(ctk.CTk):
    def __init__(self) -> None:
        ...
        self._analysis_export_start: int = 0
        self._analysis_export_step: int = 1
        self._fft_job_id: int = 0
        self._export_job_id: int = 0
    ...
    def _handle_logic_message(self, msg: Any) -> None:
        if isinstance(msg, UiMessageValidateResult):
            ...
            self._analysis_export_start = int(msg.start)
            self._analysis_export_step = int(max(1, msg.step))
            self.analysis_status_label.configure(
                text=(
            ...
            self._open_plot_popup(
                key="before",
                title="График До",
                matrix=msg.before_preview_norm,
    ...
    def _start_data_read_request(self, start: int, end: int, step: int) -> None:
        ...
        if cached is not None:
            ...
            self._analysis_export_source = cached["export_matrix"]
            self._analysis_export_start = int(start)
            self._analysis_export_step = int(max(1, step))
            self.label_data_result.configure(
            ...
            self._update_home_before_from_matrix(
                cached["plot_matrix"],
    ...
    def _apply_data_read_result(self, msg: UiMessageReadDataResult) -> None:
        ...
        use_full = bool(msg.keep_full_matrix and msg.full_matrix is not None)
        plot_matrix = msg.full_matrix if use_full else msg.preview_matrix_norm
        self._analysis_export_source = msg.full_matrix if msg.full_matrix is not None else msg.preview_matrix
        self._analysis_export_start = int(msg.start)
        self._analysis_export_step = int(max(1, msg.step))
        ...
        try:
            ...
            self._store_data_range_cache(
                cache_key,
                {
                    ...
                    "plot_matrix_normed": (not use_full),
                    "export_matrix": self._analysis_export_source,
                    "message": text,
                },
            )
        except Exception:
    ...
    def _update_home_before_from_matrix(self, matrix: Any, normalized: bool = False) -> None:
    ...
    def _open_plot_popup(
        ...
        self._plot_matrix_on_ax(ax, matrix, trace_start, trace_step, key=key, normalized=normalized)
    ...
    def _redraw_home_if_ready(self) -> None:
        ...
        if self._home_locked_by_selection and self._analysis_export_source is not None:
            self._update_home_before_from_matrix(self._analysis_export_source, normalized=False)
        else:
            self._request_home_window_read(int(self._home_window_start), force=True)
    ...
    def _open_fourier_spectrum_popup(self) -> None:
        ...
        if arr.ndim != 2 or arr.size == 0:
            ...
            return
        self._fft_job_id += 1
        job_id = self._fft_job_id
        self.analysis_status_label.configure(text="Расчёт FFT...", text_color=C.STATUS_PENDING)
        ...
        def worker_fft() -> None:
            try:
                ...
                self.after(0, lambda: self._on_fourier_error(job_id, str(ex)))
                return
            self.after(0, lambda: self._show_fourier_popup(job_id, freq, spec))

        threading.Thread(target=worker_fft, name="FFTWorker", daemon=True).start()

    def _on_fourier_error(self, job_id: int, message: str) -> None:
        if job_id != self._fft_job_id:
            return
        self.analysis_status_label.configure(text=f"Ошибка FFT: {message}", text_color=C.STATUS_WARN)

    def _show_fourier_popup(self, job_id: int, freq: Any, spec: Any) -> None:
        if job_id != self._fft_job_id:
            return
        try:
            from matplotlib.backends.backend_tkagg import FigureCanvasTkAgg
    ...
    def _export_analysis_table(self) -> None:
        ...
        matrix = self._analysis_export_source
        if matrix is None:
            messagebox.showinfo(
                "Выгрузка",
                "Нет данных для выгрузки: сначала выберите данные или запустите обработку.",
                parent=self,
            )
            return
        ...
        arr = np.asarray(matrix, dtype=np.float32)
        if arr.ndim != 2 or arr.size == 0:
            messagebox.showwarning("Выгрузка", "Недостаточно данных для выгрузки.", parent=self)
            return
        trace_start = int(self._analysis_export_start)
        trace_step = int(max(1, self._analysis_export_step))
        self._export_job_id += 1
        job_id = self._export_job_id
        self.analysis_status_label.configure(text="Выгрузка данных...", text_color=C.STATUS_PENDING)

        def worker_export() -> None:
            try:
                import csv

                n_traces, n_samples = int(arr.shape[0]), int(arr.shape[1])
                with open(path, "w", newline="", encoding="utf-8-sig") as f:
                    writer = csv.writer(f, delimiter=";")
                    cols = ["Время"] + [f"Трасса {trace_start + i * trace_step}" for i in range(n_traces)]
                    writer.writerow(cols)
                    for sample_idx in range(n_samples):
                        row = [sample_idx]
                        row.extend(float(arr[tr_idx, sample_idx]) for tr_idx in range(n_traces))
                        writer.writerow(row)
            except Exception as ex:
                self.after(0, lambda: self._on_export_error(job_id, str(ex)))
                return
            self.after(0, lambda: self._on_export_done(job_id, path))

        threading.Thread(target=worker_export, name="CsvExportWorker", daemon=True).start()

    def _on_export_done(self, job_id: int, path: str) -> None:
        if job_id != self._export_job_id:
            return
        self.analysis_status_label.configure(text=f"Выгрузка завершена: {path}", text_color=C.STATUS_OK)

    def _on_export_error(self, job_id: int, message: str) -> None:
        if job_id != self._export_job_id:
            return
        self.analysis_status_label.configure(text=f"Ошибка выгрузки: {message}", text_color=C.STATUS_WARN)
